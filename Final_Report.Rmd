...
s1 <- senate_polls %>%
  ...
  ggplot(aes(x = as.Date(end_date, format = "%m/%d/%y"), y = pct, color = candidate_party)) + geom_line() + facet_wrap(~state) + labs(title = "Polls by closer senate races", caption="Fig 1.4 Polls by closer senate races") + scale_colour_manual(values = c("DEM" = "#123adb", "REP" = "#E01414")) + xlab("Dates by Year") + ylab("Percentage estimate of being elected") + theme(plot.title = element_text(hjust = 0.5), , plot.caption = element_text(hjust = 0.5)) + theme(axis.text.x=element_blank(),
...
n0 <- NC_senate %>%
  ggplot(aes(x = as.Date(end_date, format = "%m/%d/%y"), y = pct, color = as.factor(candidate_party))) + geom_point() + scale_colour_manual(values = c("DEM" = "#123adb", "REP" = "#E01414")) + xlab("Dates by Poll") + ylab("Percentage estimate of being elected") + theme(plot.title = element_text(hjust = 0.5), , plot.caption = element_text(hjust = 0.5)) + labs(title = "NC Senate Race", caption="Fig 1.5 Polls for individual Senate races labelled by part", legend = "Candidate Party") + geom_vline(xintercept=as.Date('2020-10-06'))
```
...
p2
```
For senate data, we look at the senate races and their probabilities for being elected (Democrat or Republican). We can observe that overall Democratic candidates tend to have high percentages for being elected into the senate over Republican candidates, and this distinction becomes more prominent closer to the election date.  In Fig 1.5, we can also see the percentages for the Republican incumbent Tillis and Democratic opposer Cunningham in the NC Senate race. The vertical black line represents October 3, the day Tillis tested positive for COVID-19 and Cunningham admitted sending extramarital romantic texts (soon after proven to be part of an affair). It appears polls ending after this date are unaffected by these developments, so perhaps the polls are missing the state's true sentiment towards the developments.
...
The Linzer model is a Bayesian forecast model that specifies how preferences evolve over time and how events are noisy measurements of the underlying preferences. We adapt the Linzer model by replacing the time-varying deviation parameters with priors for the variance between state preferences, the variance between poll observations in a state, and the mean of state preferences on election day (more information on the priors is in Appendix A). We choose to make this modification to decrease the time needed to converge and incorporate hierarchical structures.
...
Sensitivity analysis was again conducted by changing the prior for $\mu_0$ in the model. We tried increasing and decreasing the variance of $\mu_0$ (to 3.5 and 12 respectively), but had no impact on our prediction. We still got estimated probabilities of 0%. 

We also plotted the beta estimates over time along with their credible intervals to see if the trend was a suitable fit within the intervals, but considering the extremeness of the predictions made by our model with respect to the electoral college, the relationship between our point estimates and their credible intervals had no meaningful interpretations in terms of sensitivity analysis.
...
Sensitivity analysis was again conducted by changing the prior for $\mu_0$ in the model. We tried increasing and decreasing the variance of $\mu_0$ to the same values, but had almost no impact on our prediction. We still got estimated probabilities of less than 1%. 
...
\end{aligned}$$.
Moreover, we use priors 
$$\begin{aligned}
\nu_y \sim Un(0, 100) \\
\tau_y \sim Un(0, 100) \\
\nu_\beta \sim Un(0, 100) \\
\tau_\beta \sim Un(0, 100) \\
\mu_0 \sim N(50, 7.5) \\
\sigma^2_0 \sim IG(0.5, 0.5)
\end{aligned}$$.

Since $\nu_y, \nu_\beta$ are degrees of freedom, we believe $Un(0, 100)$ are relatively uninformative. Similarly, since $\tau_y, \tau_\beta$ are the prior guess for the expected variance between states, we believe $Un(0, 100)$ are relatively uninformative. Additionally, we believe states (especially swing states) ought to have mean $\mu_0$ of 50\% on average and that 95\% of the time, $mu_0$ should be within 35 to 65\%. Finally, the variance prior for each state is given a high prior variance.

We only consider polls ending 200 days or less before the election. We include polls from all 50 states. The Republican presidential candidate wins the electoral college for state $j$ if $w_j := \beta_{j,1} > 0.5$. The same candidate wins the election if 
$$\sum_{j=1}^{50} w_j ec_j \geq 270$$, where $ec_j$ is the number of electoral votes for state $j$.

### Estimates From Model

```{r, echo = FALSE, warning = F, messge = F}
...
```{r run_model,eval=TRUE, echo = FALSE, message = F, warning = F}
...
```{r, echo = FALSE, warning = F, message = F}
max_day = 60 # TODO modify (max for plotting/saving parameters)
beta_j_strings = paste0('beta[', seq(1, length(states)), ',')
all_beta_strings = c()
for (beta_j_string in beta_j_strings) {
  all_beta_strings = c(all_beta_strings, paste0(beta_j_string, seq(1, max_day), ']'))
}
jags_r_all <- jags(data = jags_data_trump,
                  model.file = model,
                  parameters.to.save = all_beta_strings,
                  n.iter = 1000)

poll_plot_data <- tibble(y=jags_data_trump$y,t=jags_data_trump$t %>% as.integer(), state = states[jags_data_trump$r])

beta_plot_data <- lapply(1:N_states,function(st){
  sims <- jags_r_all$BUGSoutput$sims.list$beta[,st,1:max_day]
  data.frame(state = states[st],t=1:max_day,mean=colMeans(sims),lb=apply(sims,2,quantile,probs=.025),ub=apply(sims,2,quantile,probs=.975))
}) %>% 
  bind_rows()

good_states = states[1:length(states)] # TODO modify to plot less things
...
left_join(beta_plot_data,poll_plot_data) %>% 
  ...
  geom_point(aes(y=y)) + 
  scale_x_reverse() + 
  ylim(10, 90) + # TODO remove to show large uncertainty before first poll
  facet_wrap(~ state)
```
...
NC Voter Data: (https://dl.ncsbe.gov/?prefix=data/) 
...
Census Data: (https://opportunityinsights.org/data/?geographic_level=0&topic=0&paper_id=0#resource-listingNeighborhood)

Characteristics by county in North Carolina. 
...
Ballotpedia: (https://ballotpedia.org/United_States_House_of_Representatives_elections_in_North_Carolina,_2020)

Used in North Carolina House Model
Variables used: Incumbent Party for NC Congressional Races


North Carolina County Data: (http://speakermoore.com/new-n-c-congressional-districts-approved-state-house-representatives/)

Used in North Carolina House and Senate Models
Variables used: What district all the counties in North Carolina are a part of


North Carolina 2016 Election Data: (https://en.wikipedia.org/wiki/2016_United_States_presidential_election_in_North_Carolina#:~:text=The%202016%20United%20States%20presidential,of%20the%202016%20general%20election.)
...
```{r, echo = FALSE, include = F, message = F, include = F}
set.seed(2016)
...
```{r, echo = FALSE, include = F, message = F, warning = F}
w1 = ifelse(b1r > 50, 1, 0)
w2 = ifelse(b2r > 50, 1, 0)
w3 = ifelse(b3r > 50, 1, 0)
w4 = ifelse(b4r > 50, 1, 0)
