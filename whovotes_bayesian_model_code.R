...
brmsfit <- brm(data = modeling_data_sample, family = binomial,
               n | trials(n_registered) ~ 1 + 1 + race_code + gender_code + ethnic_code + age_group + party_cd + presidential + 
                 race_code:gender_code + race_code:ethnic_code + race_code:age_group + ethnic_code:age_group + ethnic_code:gender_code 
                 + (1 + race_code + gender_code + ethnic_code + age_group + party_cd + presidential + race_code:gender_code + race_code:ethnic_code + race_code:age_group + ethnic_code:age_group + ethnic_code:gender_code||congressional_dist),
               prior = c(prior(normal(0, 10), class = Intercept),
                         prior(normal(0, 10), class = b),
                         prior(cauchy(0, 10), class = sd)),
               iter = 2500, warmup = 500, cores = 2, chains = 2,
               seed = 10, file = "whovotes_bayesian_model")
