...
```{r}
ethnicity <- read.csv("/Users/ashleymurray/Downloads/ethnicity.csv")
ethnicity <- ethnicity %>%
  select(-c(fips, region, cog, msa))
ethnicity <- ethnicity %>%
  select(c(county, hisp, race, sex, tot2016, tot2018))
ethnicity <- filter(ethnicity, hisp != "Total")
ethnicity <- filter(ethnicity, race != "Total")
ethnicity <- filter(ethnicity, sex != "Total")
```

```{r}
alldata <- read.csv("/Users/ashleymurray/Downloads/alldata.csv")
alldata <- alldata %>%
